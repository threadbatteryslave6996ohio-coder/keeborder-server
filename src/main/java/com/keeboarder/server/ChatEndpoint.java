...
public class ChatEndpoint {
    ...
    public static void initialize(RedisClientCache cache) {
        redisCache = cache;
    }

    @OnOpen
    public void onOpen(Session session) throws IOException {
        sendJson(session, createSystemMessage("register_required", "Send a registration payload with type=register, clientId and name."));
    }
    ...
    private void handleRegister(Session session, Message msg) throws IOException {
        if (msg.clientId == null || msg.clientId.isBlank()) {
            msg.clientId = UUID.randomUUID().toString();
        }
    ...
    private void handlePersonalMessage(Session session, Message msg) throws IOException {
        ...
        delivered.addProperty("fromClientId", msg.clientId);
        delivered.addProperty("content", msg.content);
        sendJson(target, delivered);
    }

    private void handleBroadcastMessage(Session session, Message msg) {
        JsonObject broadcast = new JsonObject();
        broadcast.addProperty("type", "broadcast");
        broadcast.addProperty("fromClientId", msg.clientId);
